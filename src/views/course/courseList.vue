  ...
  <el-col :span="20">
    <el-col>
      ...
      <el-form v-model="courseSearchForm" :inline="true" style="width: 100%; margin-top: 30px;margin-left:-240px">
        ...
        <el-button type="primary" size="medium" @click="dialogFormVisible=true" icon="el-icon-plus" :round="true"
                   style="margin-left:100px">新增课程
        </el-button>
        <el-button type="primary" size="medium" @click="upShelf(selectRows)" icon="el-icon-top" style="" :round="true">
          上架
        </el-button>
        <el-button type="info" size="medium" @click="" @click="downShelf(selectRows)" icon="el-icon-bottom" :round="true">下架</el-button>
      </el-form>
    </el-col>
    <!-- 表格 -->
    <el-table :data="courseDataList" border class="courseDataTable" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="courseType" label="课程分类" width="180px" align="center"
                       :formatter="courseTypeFormatter"></el-table-column>
      ...
      <el-table-column prop="courseState" label="上架状态" align="center" :formatter="courseStateFormatter"
                       width="120px"></el-table-column>
      <el-table-column label="操作" align="center" width="240px">
        <template slot-scope="scope">
          <el-button size="small" plain style="font-size: 13px;background-color: #8cc5ff;">
            <router-link to="/manage/courseInfo">课程详情</router-link>
          </el-button>
...
<script>
  export default {
    ...
    methods: {
      ...
      //courseTypeFormatter
      courseTypeFormatter (row, column, cellValue, index) {
        let courseName
        this.courseTypeOptions.forEach(e => {
          if (e.code == cellValue) {
            courseName = e.name
            return
          }
      ...
      queryCourseList (page, size) {
        let url = this.HOME + '/course/edu/course/list?pageNum=' + page + '&pageSize=' + size
        this.HTTP.get(url, null).then(res => {
          if (res.success) {
            this.courseDataList = res.data.list
      ...
      handleDelete (index) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          let url = this.HOME + '/course/edu/course/delete/' + this.courseDataList[index].id
          this.HTTP.get(url, null).then(res => {
            if (res.success == true) {
              this.reload()
              this.$message({
      ...
      // 上架课程
      upShelf (selectRows) {
        let selectIds = this.getSelectIds(selectRows)
        let url = this.HOME + '/course/edu/course/upShelf'
        this.HTTP.post(url,selectIds).then(res => {
          if (res.success){
            this.reload()
      ...
      downShelf(selectRows){
        let selectIds = this.getSelectIds(selectRows)
        let url = this.HOME + '/course/edu/course/downShelf'
        this.HTTP.post(url,selectIds).then(res => {
          if (res.success){
            this.reload()
